...
let getData = function
  ...
  | PTreeInfo NoInfo ->
      (idNoInfo, [], [], [], [], [], [], [], [], [], [])
  | PTreePropTy (PropVar (fi, x, _)) ->
      (idPropVar, [fi], [], [], [], [x], [], [], [], [], [])
  | PTreePropTy (PropSet (fi, {nonseq; unique})) ->
      let ns = if nonseq then 1 else 0 in
      let uq = if unique then 1 else 0 in
      (idPropSet, [fi], [], [], [], [], [ns; uq], [], [], [], [])
  | _ ->
      failwith "The AST node is unknown"

let getId t =
  let id, _, _, _, _, _, _, _, _, _, _ = getData t in
  id

let getTerm t n =
  let _, _, _, _, lst, _, _, _, _, _, _ = getData t in
  PTreeTm (List.nth lst n)

let getType t n =
  let _, _, _, lst, _, _, _, _, _, _, _ = getData t in
  PTreeTy (List.nth lst n)

let getString t n =
  let _, _, _, _, _, lst, _, _, _, _, _ = getData t in
  List.nth lst n |> Intrinsics.Mseq.Helpers.of_ustring

let getInt t n =
  let _, _, _, _, _, _, lst, _, _, _, _ = getData t in
  List.nth lst n

let getFloat t n =
  let _, _, _, _, _, _, _, lst, _, _, _ = getData t in
  List.nth lst n

let getListLength t n =
  let _, _, lst, _, _, _, _, _, _, _, _ = getData t in
  List.nth lst n

let getConst t n =
  let _, _, _, _, _, _, _, _, lst, _, _ = getData t in
