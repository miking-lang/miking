...
atom:
  | atom DOT proj_label
      { let fi = mkinfo (tm_info $1) (fst $3) in
        ...
        TmMatch(fi,$1,PatRecord(fi,Record.singleton (snd $3) (PatNamed(fi,NameStr(id,Symb.nosym)))),
                                TmVar(fi,id,Symb.nosym), TmNever(fi)) }
  | LPAREN seq RPAREN
  ...
  | var_ident                { TmVar($1.i,$1.v,Symb.nosym) }
...
name:
  | var_ident
      { if $1.v =. us"_"
        ...
        else ($1.i, NameStr($1.v,Symb.nosym)) }
...
pat_atom:
  ...
  | con_ident pat_atom
      { PatCon(mkinfo $1.i (pat_info $2), $1.v, Symb.nosym, $2) }
  | patseq
