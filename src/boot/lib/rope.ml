...
let uninit_bigarray (k : ('a, 'b) kind) (n : int) : ('a, 'b) ba =
  Array1.create k c_layout n

let make_int_bigarray (n : int) (v : int) : int_ba t =
  let a = uninit_bigarray Int n in
  Array1.fill a v ; ref (Leaf a)

let make_float_bigarray (n : int) (v : float) : float_ba t =
  let a = uninit_bigarray Float64 n in
  Array1.fill a v ; ref (Leaf a)

let make_array (n : int) (v : 'a) : 'a array t = ref (Leaf (Array.make n v))

let empty_bigarray (k : ('a, 'b) kind) : ('a, 'b) ba t =
  ref (Leaf (uninit_bigarray k 0))
...
let concat_array (l : 'a array t) (r : 'a array t) : 'a array t =
  ref (Concat {lhs= !l; rhs= !r; len= length_array l + length_array r})

let concat_bigarray (l : ('a, 'b) ba t) (r : ('a, 'b) ba t) : ('a, 'b) ba t =
  ref (Concat {lhs= !l; rhs= !r; len= length_bigarray l + length_bigarray r})

let collapse_array (s : 'a array t) : 'a array =
...
let collapse_bigarray (s : ('a, 'b) ba t) : ('a, 'b) ba =
  let rec collapse dst s i =
    match s with
    | Leaf a ->
...
let cons_bigarray (v : 'a) (s : ('a, 'b) ba t) : ('a, 'b) ba t =
  let a = uninit_bigarray (_bigarray_kind !s) 1 in
  Array1.unsafe_set a 0 v ;
  ref (Concat {lhs= Leaf a; rhs= !s; len= length_bigarray s + 1})

let snoc_array (s : 'a array t) (v : 'a) : 'a array t =
  ref (Concat {lhs= !s; rhs= Leaf [|v|]; len= length_array s + 1})
...
let split_at_bigarray (s : ('a, 'b) ba t) (idx : int) :
  ...
  if idx = 0 then (empty_bigarray k, s)
  else if idx = n then (s, empty_bigarray k)
  else
    let lhs = uninit_bigarray k idx in
    let rhs = uninit_bigarray k (n - idx) in
    let rec fill s i =
      match s with
      | Leaf a ->
          let n = Array1.dim a in
...
let sub_array (s : 'a array t) (from : int) (len : int) : 'a array t =
  ...
  if n = 0 then empty_array
    ...
    let len = if from + len < n then len else n - (from + len) in
    let dst = Array.make len (get_array s 0) in
    let rec fill s i =
      match s with
      | Leaf a ->
          let n = Array.length a in
          let src_offset = if i < from then from - i else 0 in
          let copy_len = min (n - src_offset) len in
          Array.blit a src_offset dst i copy_len ;
          i + n
      | Concat {lhs; rhs; _} ->
          fill rhs (fill lhs i)
    in
...
let sub_bigarray (s : ('a, 'b) ba t) (from : int) (len : int) : ('a, 'b) ba t =
  ...
  let n = length_bigarray s in
  let len = if from + len < n then len else n - (from + len) in
  let dst = uninit_bigarray k len in
  let rec fill s i =
    match s with
    | Leaf a ->
        let n = Array1.dim a in
        let src_offset = if i < from then from - i else 0 in
        let copy_len = min (n - src_offset) len in
        let dst_sub = Array1.sub dst i copy_len in
        Array1.blit a dst_sub ; i + n
    | Concat {lhs; rhs; _} ->
        fill rhs (fill lhs i)
  in
...
let combine_array_array (l : 'a array t) (r : 'b array t) : ('a * 'b) array t =
  ...
  if ln != rn then failwith "combine: sequences of different length"
    ...
    let a1, a2 = (collapse_array l, collapse_array r) in
...
let combine_array_bigarray (l : 'a array t) (r : ('b, 'c) ba t) :
  ...
  if ln != rn then failwith "combine: sequences of different length"
    ...
    let a1, a2 = (collapse_array l, collapse_bigarray r) in
...
let combine_bigarray_array (l : ('a, 'c) ba t) (r : 'b array t) :
  ...
  if ln != rn then failwith "combine: sequences of different length"
    ...
    let a1, a2 = (collapse_bigarray l, collapse_array r) in
...
let combine_bigarray_bigarray (l : ('a, 'c) ba t) (r : ('b, 'd) ba t) :
  ...
  if ln != rn then failwith "combine: sequences of different length"
    ...
    let a1, a2 = (collapse_bigarray l, collapse_bigarray r) in
...
module Convert = struct
  let to_array_array (s : 'a array t) : 'a array = collapse_array s
