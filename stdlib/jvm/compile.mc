...
lang MExprJVMCompile = MExprAst + JVMAst
    ...
    | TmConst { val = val } -> 
        ...
        in { env with bytecode = snoc env.bytecode bc }
    | TmApp { lhs = lhs, rhs = rhs } -> 
        match lhs with TmConst { val = val } then 
            match val with CPrint _ then 
                -- push this to stack
                let this = { env with bytecode = snoc env.bytecode (createBApply "GETSTATIC" "java/lang/System" "out" "Ljava/io/PrintStream;") } in
                -- push args to stack
                let args = (toJSONExpr this rhs) in
                -- call function
                { args with bytecode = snoc args.bytecode (createBApply "INVOKEVIRTUAL" "java/io/PrintStream" "print" "(Ljava/lang/String;)V") }
            else 
