-- CFA framework for MExpr. The algorithm is loosely based on Table 3.7 in the
-- book "Principles of Program Analysis" (Nielson et al.).
--
-- Some details:
...
lang CFA = Ast + LetAst + MExprIndex + MExprPrettyPrint
  ...
  | graph ->
    ...
    let f = lam env. lam e: (IName,Ctx,AbsVal).
      ...
      match absValToString graph env e.2 with (env,av) in
      (env,join ["(", n, ", ", av, ")"]) in
    ...
    match mapAccumL (lam env: PprintEnv. lam b:(IName,Map Ctx (Set AbsVal)).
        ...
        let printAbsVals = lam env. lam s: Set AbsVal.
          mapAccumL (absValToString graph) env (setToSeq s)
        in
    ...
    match mapAccumL (lam env: PprintEnv. lam b:(IName,Map Ctx [Constraint]).
        ...
        let printCstrs = lam env. lam cstrs: [Constraint].
          mapAccumL (constraintToString graph) env cstrs
        in
...
lang BaseConstraint = CFA
  ...
  | CstrInit { lhs = lhs, rhs = rhs } ->
    ...
    match pprintVarINameCtx graph.im env rhs with (env,rhs) in
    (env, join ["{", lhs, "}", " ⊆ ", rhs])
  | CstrDirect { lhs = lhs, rhs = rhs } ->
    match pprintVarINameCtx graph.im env lhs with (env,lhs) in
    match pprintVarINameCtx graph.im env rhs with (env,rhs) in
    (env, join [lhs, " ⊆ ", rhs])
...
lang LamCFA = CFA + BaseConstraint + LamAst
  ...
  | AVLam { ident = ident, bident = bident } ->
    ...
    match pprintVarIName graph.im env bident with (env,bident) in
...
lang ConstCFA = CFA + ConstAst + BaseConstraint + Cmp
  ...
  | AVConst { id = id, const = const, args = args } ->
    ...
    match mapAccumL (pprintVarINameCtx graph.im) env args with (env,args) in
    let args = strJoin ", " args in
    match pprintVarINameCtx graph.im env id with (env,id) in
...
lang AppCFA = CFA + ConstCFA + BaseConstraint + LamCFA + AppAst + MExprArity
  ...
  | CstrLamApp { lhs = lhs, rhs = rhs, res = res } ->
    match pprintVarINameCtx graph.im env lhs with (env,lhs) in
    match pprintVarINameCtx graph.im env rhs with (env,rhs) in
    match pprintVarINameCtx graph.im env res with (env,res) in
...
lang RecordCFA = CFA + BaseConstraint + RecordAst
  ...
  | AVRec { bindings = bindings } ->
    match mapMapAccum (lam env. lam k. lam v.
        match pprintVarINameCtx graph.im env v with (env, v) in
...
lang SeqCFA = CFA + BaseConstraint + SeqAst
  ...
  sem absValToString graph (env: PprintEnv) =
  | AVSeq { names = names } ->
    match mapAccumL (pprintVarINameCtx graph.im) env (setToSeq names)
    with (env,names) in
...
lang DataCFA = CFA + BaseConstraint + DataAst
  ...
  | AVCon { ident = ident, body = body } ->
    match pprintConINameCtx graph.im env ident with (env,ident) in
    match pprintVarINameCtx graph.im env body with (env,body) in
    (env, join [ident, " ", body])
...
lang MatchCFA = CFA + BaseConstraint + MatchAst
  ...
  | CstrMatch { id = id, pat = pat, target = target } ->
    ...
    match pprintVarINameCtx graph.im env target with (env, target) in
...
lang ExtCFA = CFA + ExtAst
  ...
  | AVExt { ext = ext, args = args } ->
    ...
    match mapAccumL (pprintVarINameCtx graph.im) env args with (env,args) in
    let args = strJoin ", " args in
    match pprintVarINameCtx graph.im env ext with (env,ext) in
    (env, join [ext, "(", args, ")"])
