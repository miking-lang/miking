...
type LambdaLiftSolution =
  { vars : Map Name Type
  , tyVars : Map Name Kind
  }
type OrderedLamLiftSolution =
  { vars : [(Name, Type)]
  , tyVars : [(Name, Kind)]
  }
...
lang LambdaLiftReplaceCapturedParameters = MExprAst + MExprSubstitute
  ...
  | ast ->
    let newNamesForSolution
      ...
      = lam sol.
        let substs = mapUnion
          (mapMapWithKey (lam k. lam. nameSetNewSym k) sol.vars)
          (mapMapWithKey (lam k. lam. nameSetNewSym k) sol.tyVars) in
...
let expected = preprocess (bindall_ [
  ...
  unit_]) in
printLn "Thing";
printLn (expr2str recursiveSystem);
printLn "Thing2";
printLn (expr2str expected);
printLn "Thing3";
printLn (expr2str (liftLambdas recursiveSystem));
