...
-- Marshals tm of type fromTy to type toTy, returning {cost : Int, tm : Expr},
-- where tm is the result and cost the cost of the
-- marshaling, repsectivly.
let externalMarshal = lam tm. lam fromTy. lam toTy.
  recursive
      ...
      match tt with (TyVar _, _) then
        ...
        if neqi (length fromTys) (length toTys) then
          ...
          let tmp : [{tm : Expr, cost : Int}] =
            zipWith (lam n. lam tt. recur (nvar_ n) tt) names argtts
          in
