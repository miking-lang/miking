  ...
  let replacement : (Map VarPattern (Name, Expr)) -> Expr = lam matches.
    ...
    match branchExpr with TmMatch {els = els} then
      ...
      let subMap = mapFromSeq nameCmp [
        (headPair.0, lam info.
          TmVar {ident = x, ty = tyWithInfo info (ty headPair.1), info = info, frozen = false})
      ] in
  ...
  let replacement : (Map VarPattern (Name, Expr)) -> Expr = lam matches.
    ...
    match branchExpr with TmMatch {els = els} then
      match fExpr with TmSeq {tms = [fResultVar]} then
        ...
        let subMap = mapFromSeq nameCmp [
          (headFst.0, lam info.
            TmVar {ident = x, ty = tyWithInfo info (ty headFst.1), info = info, frozen = false}),
          (headSnd.0, lam info.
            TmVar {ident = y, ty = tyWithInfo info (ty headSnd.1), info = info, frozen = false})
        ] in
        let els = substituteVariables els subMap in
        let els = eliminateUnusedLetExpressions (bind_ els fResultVar) in
...
let reducePattern : () -> Pattern =
  ...
  let replacement : Map VarPattern (Name, Expr) -> Expr = lam matches.
    ...
    match branchExpr with TmMatch {els = els} then
      ...
      let subMap = mapFromSeq nameCmp [
        (accPair.0, lam info.
          TmVar {ident = x, ty = tyWithInfo info (ty accPair.1), info = info, frozen = false}),
        (headPair.0, lam info.
          TmVar {ident = y, ty = tyWithInfo info (ty headPair.1), info = info, frozen = false})
      ] in
      let els = substituteVariables els subMap in
