...
-- String representation of tensor `t`, where elements are represented using
-- `estr`.
let tensorToString : (a -> String) -> Tensor[a] -> String =
lam estr. lam t.
  recursive let recur = lam indent. lam t.
    if eqi (tensorRank t) 0 then estr (tensorGetExn t [])
    else if eqi (tensorRank t) 1 then
      let elems =
        tensorFoldlSlice (lam acc. lam t. snoc acc (recur "" t)) [] t
      in
      join ["[", (strJoin ", " elems), "]"]
    else
      let newindent = snoc indent '\t' in
      let elems =
        tensorFoldlSlice (lam acc. lam t. snoc acc (recur newindent t)) [] t
      in
      join [
        "[\n",
        newindent,
