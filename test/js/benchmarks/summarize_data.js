...
function main(args) {
  ...
  fs.writeFileSync(summaryFile, `"Benchmark name"`.padEnd(longestName + 4, " ") + `"mi eval"\t"boot eval"\t"node (manual)"\t"node (compiled)"\t"node (compiled+opt)"\t"bun (manual)"\t"bun (compiled)"\t"bun (compiled+opt)"\n`);
